...
# :transaction cleanup in a `config.around { DatabaseCleaner.cleaning
# { ... } }` block. The BEGIN / SAVEPOINT / ROLLBACK queries the
# cleaner emits inside the around hook fire `sql.active_record` events
# INSIDE the per-example bucket window, so the schema-subscriber
# attributes db/schema.rb to every example wrapped by the cleaner.
# Same widening shape as the :truncation / :deletion strategies,
# different SQL (transactional rollback rather than table-level
# clear).
#
# Three assertions:
...
module WideArSchemaDbCleanerTransactionSpecHelpers
  TRACER_ENV = {
    'RSPEC_TRACER' => '1',
    'RSPEC_TRACER_RAILS_TRANSACTIONAL' => 'false',
    'RSPEC_TRACER_DB_CLEANER_STRATEGY' => 'transaction'
  }.freeze
...
RSpec.describe 'wide AR schema attribution under DatabaseCleaner :transaction' do
  include WideArSchemaDbCleanerTransactionSpecHelpers

  before(:all) do
    FixtureBundleHelper.ensure_bundle_and_db
    FixtureBundleHelper.clear_tracer_state

    out, status = WideArSchemaDbCleanerTransactionSpecHelpers.run_rspec_in_fixture
    ...
    @ar_ids = WideArSchemaDbCleanerTransactionSpecHelpers::AR_DESCRIPTIONS.to_set do |desc|
      WideArSchemaDbCleanerTransactionSpecHelpers.example_id_matching(@cold_all_examples, desc)
    end
    @pure_id = WideArSchemaDbCleanerTransactionSpecHelpers.example_id_matching(
      @cold_all_examples, WideArSchemaDbCleanerTransactionSpecHelpers::PURE_DESCRIPTION
    )
  end
  ...
  describe 'cold attribution' do
    it 'attributes db/schema.rb to every AR-touching example' do
      schema_dependents = @cold_reverse_dependency
        .fetch(WideArSchemaDbCleanerTransactionSpecHelpers::SCHEMA_FILE_NAME, [])
        .to_set
  ...
  describe 'warm filter after schema mutation' do
    it 're-runs every AR-touching example and skips the pure-compute one' do
      ...
      File.open(schema_path, 'ab') { |f| f.write(WideArSchemaDbCleanerTransactionSpecHelpers::MUTATION_MARKER) }
