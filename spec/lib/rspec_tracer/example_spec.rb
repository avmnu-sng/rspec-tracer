...
# stability-contract comment in lib/rspec_tracer/example.rb):
...
# ride along in the returned Hash for the reporter location columns
# but are excluded from the digest, so a no-op line-shifting edit
# must not flip the id.
#
# For an UNNAMED example (`it { }` / `specify { }` / `example { }`)
# RSpec's `description` is the line-bearing "example at <path>:<line>"
# fallback, which would re-leak the line number into the digest;
# the digest substitutes the example's ordinal among its group's
# unnamed examples instead.
#
...
RSpec.describe RSpecTracer::Example do
  ...
  # each test vary exactly one identity input. The example_group
  # double responds to `description` but NOT `name`: `from` must
  # read `.description`, and a `.description` -> `.name` mutation
  # raises here.
  #
  # `raw_description` is metadata[:description] - RSpec's RAW explicit
  # description string ('' or nil for an unnamed example). It defaults
  # to `description`, so a plain build_example models a NAMED example;
  # pass `raw_description: ''` for an unnamed one. `siblings` overrides
  # example_group.examples (default [self]) so `unnamed_description`
  # can take the intra-group ordinal.
  ...
  # Builds N example doubles sharing one example_group. Each entry in
  # `raw_descriptions` is one example's metadata[:description]
  # ('' / nil => unnamed). Every example's example_group.examples is
  # the full ordered list, so `unnamed_description` can take ordinals.
  # `line_base` shifts every line number uniformly (a no-op-edit
  # simulation) without changing definition order.
  def build_group(raw_descriptions, group_description: 'G', file_path: '/proj/spec/g_spec.rb', line_base: 0)
    ...
    raw_descriptions.each_with_index do |raw, idx|
      ...
      examples << double(
        ...
        metadata: {
          description: raw, file_path: file_path, rerun_file_path: file_path,
          line_number: line, shared_group_inclusion_backtrace: []
        }
      )
    end
    examples
  ...
  describe '.from example_group identity' do
    it 'reads example_group.description (the user-supplied string), not example_group.name' do
      # The build_example group double responds to :description only;
      # a `.description` -> `.name` regression would raise an
  ...
  describe '.from digest EXCLUDES line numbers' do
  ...
  describe '.from describe-block edge cases' do
    it 'handles an anonymous describe (empty description) without crashing' do
  ...
  describe '.from unnamed examples (it { } / specify { } / example { })' do
    # An unnamed example has no explicit description string, so RSpec's
    # `description` method returns the line-bearing
    # "example at <path>:<line>" fallback. `Example.from` substitutes
    # the example's ordinal among its group's UNNAMED examples so the
    # line number never enters the digest.
    ...
    it 'is line-independent — a no-op edit above an it { } no longer flips the id' do
    ...
    it 'keeps RSpec description / full_description in the stored payload (digest-input only)' do
      ...
      # IS carried through for the reporter columns.
    ...
    it 'counts the ordinal among UNNAMED siblings only — a named sibling does not shift it' do
      # The unnamed example sits at all-index 1 here, all-index 2 there,
    ...
    it 'treats a nil metadata[:description] as unnamed' do
    ...
    it 'leaves named examples on the original digest path — the ordinal branch never touches them' do
